...
from .config import LLM_MODEL, VOICE_ASSISTANT_NAME


class QAAssistant:
    """Natural language Q&A assistant - Complete Roman English Support"""
    
    def __init__(self):
        self.loader = DataLoader()
        self.loader.load_all_data()
        self.llm_available = self.check_ollama()
        self.name = VOICE_ASSISTANT_NAME
    ...
    def get_context(self):
        """Get current inventory context for LLM - Roman English"""
        ...
        if not stock.empty:
            ...
            if not stock.empty:
                ...
                for _, row in top.iterrows():
                    context += f"  - {row['thread_name']} (Size: {row['size']}): {row['available']:.0f} bundles\n"
    ...
    def query(self, question):
        ...
        prompt = f"""You are an inventory management assistant. Answer in Roman English (like 'aap kaise hain' style).
...
Answer in Roman English:"""
    ...
    def quick_query(self, question):
        ...
        # ============================================================
        # ✅ STEP 1: CHECK FOR NAME QUESTION - PEHLE YEH CHECK HOGA
        # ============================================================
        name_keywords = [
            'apka naam', 'tumhara naam', 'aapka naam', 'name kya hai',
        ...
        if any(kw in question_lower for kw in name_keywords):
            return f"👋 Mera naam AlRehman hai! Main aapki AI assistant hoon."
        
        # STEP 2: CHECK FOR STOCK QUERY
        stock_keywords = [
            'stock', 'available', 'mujooda', 'baqi', 'bacha', 
            'kya', 'kitna', 'kitni', 'kya hai', 'kitna hai',
            'bache', 'bachi', 'rakam', 'maujooda',
            'stok', 'stak'
        ]
        ...
        # ============================================================
        # STEP 3: CHECK FOR SALES QUERY
        # ============================================================
        sales_keywords = [
            'sales', 'sold', 'revenue', 'bikri', 'farokht', 
            'amadani', 'bechay', 'bikay', 'bechi'
        ]
        ...
        # ============================================================
        # STEP 4: CHECK FOR CUSTOMERS
        # ============================================================
        customer_keywords = [
            'customer', 'customers', 'gahak', 'khareedar', 
            'mushteri', 'gahko'
        ]
        
        if any(kw in question_lower for kw in customer_keywords):
            if self.loader.customer_data is not None and not self.loader.customer_data.empty:
                return f"👥 Total Customers: {len(self.loader.customer_data)}"
            else:
                return "Customer ka koi data available nahi hai."
        
        # ============================================================
        # STEP 5: CHECK FOR SUPPLIERS
        # ============================================================
        supplier_keywords = [
            'supplier', 'suppliers', 'farosh', 'tajar'
        ]
        ...
        # ============================================================
        # STEP 6: CHECK FOR HELP
        # ============================================================
        help_keywords = [
            'help', 'what', 'how', 'kya', 'kaise', 'kese', 
            'batana', 'batao', 'dikhana', 'guide'
        ]
    ...
    🖨️ **Print ke liye:**
    • Pehle sawal poochiye, phir "print karo" boliye

    💡 **Roman English mein pooch sakte hain!**
    """
        
        # ============================================================
        # STEP 7: TRY LLM AS FALLBACK
        # ============================================================
...
#         # ============ ROMAN English + ENGLISH KEYWORDS ============
#         stock_keywords = [
#             'stock', 'available', 'stock', 'mojooda', 'baqi', 'baqaya', 'bacha',
