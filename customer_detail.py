...
def open_customer_detail():
    ...
    tk.Label(win, text="📋 Customer Details", font=("Arial", 18, "bold"),
             bg="#f4f6f9").pack(pady=10)

    # TREEVIEW
    columns = ("Customer Name", "Phone", "Email", "CNIC", "Company Name")
    ...
    def load_data():
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("""
                SELECT DISTINCT customer_name, phone, email, customer_cnic, company_name
                FROM stock_out
            """)
    ...
    def print_tree():
        ...
        # Optional: send to printer (Windows)
        try:
